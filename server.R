...
shinyServer(function(input, output) {
  ...
  output$iso <- renderPlot({
    ...
    p1 <-
      ...
      geom_polygon(
        data = data.frame(
          x = c(0, IC_A1, max(IC_A1, IC_M1 * 0.5), max(IC_A1, IC_M1 * 0.5)),
          y = c(IC_B1, 0, 0, max(IC_B1, IC_M1 *
                                   0.5))
        ),
        aes(x, y),
        fill = 'red',
    ...
    p2 <-
      ...
      geom_polygon(
        data = data.frame(
          x = c(0, IC_A2, max(IC_A2, IC_M2 * 0.5), max(IC_A2, IC_M2 * 0.5)),
          y = c(IC_B2, 0, 0, max(IC_B2, IC_M2 *
                                   0.5))
        ),
        aes(x, y),
        fill = 'red',
    ...
    p3 <-
      ...
      geom_polygon(
        data = data.frame(
          x = c(0, IC_A3, max(IC_A3, IC_M3 * 0.5), max(IC_A3, IC_M3 * 0.5)),
          y = c(IC_B3, 0, 0, max(IC_B3, IC_M3 *
                                   0.5))
        ),
        aes(x, y),
        fill = 'red',
  ...
  observeEvent(input$cook, {
    ...
    output$sample <- renderPlot({
      p1 <-
        qplot(X1, Y1, xlab = 'Concentration', ylab = 'Observed cell count') + theme(aspect.ratio =
                                                                                      1)
      p2 <-
        qplot(X2, Y2, xlab = 'Concentration', ylab = 'Observed cell count') + theme(aspect.ratio =
                                                                                      1)
      p3 <-
        qplot(X3, Y3, xlab = 'Concentration', ylab = 'Observed cell count') + theme(aspect.ratio =
                                                                                      1)
  ...
  observeEvent(input$run, {
    ...
    output$density <- renderPlot({
      ...
      ggplot(dens, aes(IC50_hat, fill = cover)) + geom_dotplot(dotsize =
                                                                 0.4, stackdir = "centerwhole")
    })
  })
})
